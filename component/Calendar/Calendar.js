...
Component({
  ...
  methods: {
    ...
    _setScheduleData: function (newData, oldData){
      this.setData({
        myScheduleData: newData
      })
      this.setData({
        days_array: this._setCalendarData(this.data.year, this.data.month)
      });
    },
    _formatDate: function(dateInst, fmt){
      var o = {
    ...
    _setCalendarData: function (year, month) {
      ...
      // 设置王朝日历是否有档期
      let myScheduleData = this.data.myScheduleData
      for (let i = 0; i < days.length; i++) {
        const item = days[i];
        const solarDay = item.day;
        const solarMonth = item.month;
        const solarYear = year;
        let nowDateStr = this._formatDate(new Date(solarYear, solarMonth - 1, solarDay), 'yyyy-MM-dd 00:00:00')
        // _formatDate
        if (solarDay > 0) {
          // 判断
          // console.log(myScheduleData[nowDateStr])
          if (myScheduleData[nowDateStr]) {
            days[i]['color'] = '#333333'
            let resData = myScheduleData[nowDateStr]
            let status = resData.OrderStatus

            if (status === 0) {
              days[i]['scheduleStatus'] = 0
              days[i]['schedule'] = '满'
            }
            if (status === 1) {
              days[i]['scheduleStatus'] = 1
              days[i]['schedule'] = '部'
            }
            if (status === 2) {
              days[i]['scheduleStatus'] = 2
              days[i]['schedule'] = '约'
            }
          } else {
            days[i]['color'] = '#e1e1e1'
